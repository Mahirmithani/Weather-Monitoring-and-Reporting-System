...
# fetch pressure 
@st.cache_data(ttl=30, show_spinner=False)
def fetchPressureData() -> pd.DataFrame:
    url = "https://api.anedya.io/v1/aggregates/variable/byTime"
    apiKey_in_formate = "Bearer " + apiKey
    ...
    if response.status_code == 200:
        ...
        if data_list:
            ...
            st.session_state.Currentpressure = round((data_list[1]["aggregate"]), 2)
            df = pd.DataFrame(data_list)
            # Convert timestamp to datetime and set it as the index
            df["Datetime"] = pd.to_datetime(df["timestamp"], unit="s")
            local_tz = pytz.timezone("Asia/Kolkata")  # Change to your local time zone
...
# fetch soil mositure 
@st.cache_data(ttl=30, show_spinner=False)
def fetchSoilMoistureData() -> pd.DataFrame:
    url = "https://api.anedya.io/v1/aggregates/variable/byTime"
    apiKey_in_formate = "Bearer " + apiKey
    ...
    if response.status_code == 200:
        ...
        if data_list:
            ...
            st.session_state.CurrentSoilMoisture = round((data_list[0]["aggregate"]), 2)
            df = pd.DataFrame(data_list)
            # Convert timestamp to datetime and set it as the index
            df["Datetime"] = pd.to_datetime(df["timestamp"], unit="s")
            local_tz = pytz.timezone("Asia/Kolkata")  # Change to your local time zone
...
# fech uv index
@st.cache_data(ttl=30, show_spinner=False)
def fetchUVIndexData() -> pd.DataFrame:
    url = "https://api.anedya.io/v1/aggregates/variable/byTime"
    apiKey_in_formate = "Bearer " + apiKey
    ...
    if response.status_code == 200:
        ...
        if data_list:
            ...
            st.session_state.CurrentUV = round((data_list[0]["aggregate"]), 2)
            df = pd.DataFrame(data_list)
            # Convert timestamp to datetime and set it as the index
            df["Datetime"] = pd.to_datetime(df["timestamp"], unit="s")
            local_tz = pytz.timezone("Asia/Kolkata")  # Change to your local time zone
